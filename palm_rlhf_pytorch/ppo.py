...
https://medium.com/geekculture/actor-critic-implementing-actor-critic-methods-82efb998c273#id_token=eyJhbGciOiJSUzI1NiIsImtpZCI6IjBhZDFmZWM3ODUwNGY0NDdiYWU2NWJjZjVhZmFlZGI2NWVlYzllODEiLCJ0eXAiOiJKV1QifQ..PaesyWXFZF_3aztTn55LNNN6I8TrzsuEmBixIZFWMAmIUnwJKbZ6jl0tAm_5UTEnovcuEfPzAs_QGUQmAGPctY98g7sYUxJHzD8u0wq0HBY2cZbzpTKrwXoalUACSkAFiK5zsm7xdkFD0niR-rCTZ3SwAp7qz82JqR7fT8bMCIKyT2k-LD2bxXWuuUqfstqAxR0LETeTB_tVaqWHR3kVt7_JGnae_I4zHVuD1FfRifyjwtYWozRqf6EYE0G_YzueBoD6xGcB3CVlkKGvFbR8FY0-pzcH0YSmZD5CYh08tFsgi5cTQT-DIBerZfQ6RiAFDv-Rdsgb9pXc66QQLFVweg
'''
import math
...
class ActorCritic(nn.Module):
    def __init__(
        ...
        '''
        Actor-Critic neural network - consists of an actor for generating actions and a critic network for 
        estimating state values. Both actor and critic networks can optionally use LoRA (Learnable Reweighting of Attention) layers
        The actor objective generate sequences that maximize rewards, whereas the critic is focused on accurate value estimation to assess the quality of actions 

        Methods:
        actor_parameters(): Returns the parameters of the actor network.
        critic_parameters(): Returns the parameters of the critic network.
        generate(state, max_seq_len, eos_token, return_values, **kwargs): 
            Generates actions using the actor network.
        forward(x, mask, return_values): Performs a forward pass through the 
                                         actor-critic network.

        Example:
        # Initialize an ActorCritic instance with PaLM models for actor and critic
        actor_critic = ActorCritic(palm_model_for_actor, palm_model_for_critic)

        # Generate actions based on a given state
        actions = actor_critic.generate(state, max_seq_len=512)
        '''
        super().__init__()
